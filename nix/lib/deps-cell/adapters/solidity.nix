...
rec {
  ...
  let
    ...
    # remappings.txt, so the remappings stay valid inside it. Rules that read
    # dependency sources through remappings (solidity_test) depend on it, which
    # makes those sources declared inputs instead of files read in place.
    rootBuckContent = ''
      # Auto-generated by turnkey deps-cell
      ...
      ${lib.concatMapStringsSep "\n" (
        pkg: "        \"vendor/${pkg.name}\": \"//vendor/${pkg.name}:${sanitizeName pkg.name}_all\","
      ) packages}
          },
          visibility = ["PUBLIC"],
      )
