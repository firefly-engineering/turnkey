...
let
  ...
  checkDeprecation = name: version: versionEntry:
    let
      ...
      # builtins.currentTime returns seconds since epoch (only in impure mode)
      ...
      currentDate =
        ...
        in "${toString year}-${pad2 month}-${pad2 day}";

      # ISO 8601 dates are lexicographically sortable
      eolPassed = eol != null && eol < currentDate;

      warningParts = lib.optional deprecated
        "DEPRECATED: Toolchain '${name}' version '${version}' is deprecated."
        ++ lib.optional (deprecated && deprecationMessage != null)
          "  ${deprecationMessage}"
        ++ lib.optional eolPassed
          "EOL: Toolchain '${name}' version '${version}' reached end-of-life on ${eol}.";

    in
      if warningParts == [] || suppressWarnings
      then null
