...
'';
  ...
  # ==========================================================================
  # Compose configuration generation
  # ==========================================================================
  # Generate .turnkey/compose.toml for the FUSE composition daemon.
  # Contains mount point, repo root, and cell mappings (Nix store paths).
  # ==========================================================================

  composeCellEntries = lib.concatStringsSep "\n" (
    lib.mapAttrsToList (name: cell:
      ''${name} = "${cell.derivation}"''
    ) nixCells
  );

  composeConfigContent = ''
    # Generated by turnkey - do not edit manually
    # Configuration for turnkey-composed (FUSE composition daemon)

    [mount]
    # These are placeholders — override with actual paths when starting the daemon
    ...
    layout = "buck2"
    source_dir = "root"
    cell_prefix = "external"
  '';

  composeConfig = pkgs.writeText "turnkey.compose.toml" composeConfigContent;
...
in
{
  ...
  config = lib.mkIf (cfg.enable && turnkeyCfg.enable) {
    ...
    env = {
      ...
      TURNKEY_COMPOSE_CONFIG = "${composeConfig}";
    ...
    enterShell = ''
      ...
      # Ensure .turnkey/compose.toml points to the generated compose config
      if [ -L .turnkey/compose.toml ]; then
        if [ "$(readlink .turnkey/compose.toml)" != "${composeConfig}" ]; then
          ln -sfn "${composeConfig}" .turnkey/compose.toml
          echo "turnkey: Updated compose.toml symlink"
        fi
      elif [ -e .turnkey/compose.toml ]; then
        echo "turnkey: Warning: .turnkey/compose.toml exists and is not a symlink"
        echo "         Remove it to let turnkey manage compose configuration"
      else
        ln -s "${composeConfig}" .turnkey/compose.toml
        echo "turnkey: Created compose.toml symlink"
      fi
