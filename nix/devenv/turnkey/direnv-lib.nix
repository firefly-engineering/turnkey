...
let
  ...
  # Fish completion paths (exported for fish shell integration)
  fishCompletionPaths = lib.concatStringsSep ":" (
    lib.filter (p: p != null) [
      (if cfg.registry ? tk then "${cfg.registry.tk}/share/fish/vendor_completions.d" else null)
    ]
  );
  ...
  helperFunctions = ''
    ...
    # Fish completion paths - add to fish_complete_path in your fish config:
    #   if set -q TURNKEY_FISH_COMPLETIONS
    #     for p in (string split : $TURNKEY_FISH_COMPLETIONS)
    #       if not contains $p $fish_complete_path
    #         set -a fish_complete_path $p
    #       end
    #     end
    #   end
    export TURNKEY_FISH_COMPLETIONS="${fishCompletionPaths}"
