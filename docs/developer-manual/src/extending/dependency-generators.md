...
```
Cargo.lock → rust-deps.toml → Rust deps cell (nix/lib/deps-cell) → .turnkey/rustdeps/
```
...
3. **The Rust deps cell builder** (`nix/lib/deps-cell/adapters/rust.nix`) fetches crates and generates rules.star files
