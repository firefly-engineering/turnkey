...
pub(crate) enum VirtualFile {
    ...
    BuckRoot,
    Envrc,
    /// .buckconfig inside root/ cell (sets buildfile name for the source cell)
    RootCellBuckConfig,
}
...
impl FsCore {
    ...
    pub fn get_virtual_file_content(&self, file: VirtualFile) -> String {
        ...
        match file {
            ...
            VirtualFile::RootCellBuckConfig => "[buildfile]\n    name = rules.star\n".to_string(),
        }
    }
    ...
    pub fn resolve_path(&self, path: &str) -> ResolvedPath {
        ...
        if first == self.config.source_dir_name {
            return match rest {
                ...
                Some(remainder) => {
                    // Virtual .buckconfig inside root/ cell (sets buildfile name)
